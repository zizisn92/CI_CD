...
tan() {
  eval $(real_args_or_input "$@")
  real_eval "(s($1)/c($1))" ${2:-8}
}

cotan() {
  eval $(real_args_or_input "$@")
  real_eval "(c($1)/s($1))" ${2:-8}
}
sec() {
  eval $(real_args_or_input "$@")
  real_eval "(1/c($1))" ${2:-8}
...
hypot() {
  eval $(real_args_or_input "$@")
  real_eval "sqrt(($1)^2 + ($2)^2)" ${3:-8}
}

# Inverse trig funcs
asin() {
  eval $(real_args_or_input "$@")
  real_eval "asin($1)" ${2:-8}
}
acos() {
  eval $(real_args_or_input "$@")
  real_eval "acos($1)" ${2:-8}
}
atan() {
  eval $(real_args_or_input "$@")
  real_eval "atan($1)" ${2:-8}
}

arccot() {
  eval $(real_args_or_input "$@")
  real_eval "(($PI/2)-a($1))" ${2:-8}
}
arcsin() {
  eval $(real_args_or_input "$@")
  asin "$@"
...
round() {
  eval $(real_args_or_input "$@")
  real_eval "round($1, ${2:-(scale($1)-1)})" ${2:-8}
}
