...
source ./utils/bash-check.sh                # make sure we're running bash >= 3.3

# Don't source ./utils/this list more than once per session (per version)
[[ "$LIST_UTILS_SH" = "$LIST_UTILS_VERSION" ]] && return
LIST_UTILS_SH="$LIST_UTILS_VERSION"

source ./utils/help-util.sh
source ./utils/arg-utils.sh
source ./utils/talk-utils.sh
source ./utils/text-utils.sh
